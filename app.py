...
# Prediksi dan Nama Kategori
def prediksi_kategori(resume_text):
...
def rekomendasi_job(resume_text):
    resume_text= clean_resume(resume_text)
...
def ektrak_skill(text):
    # List of predefined skills
    skills_list = [
        'Nuxt', 'Next.js', 'Vue', 'Vue.js', 'Bootstrap', 'Kotlin', 'Golang', 'Code Igniter', 'CodeIgniter', 'Python', 'Data Analysis', 'Machine Learning', 'Communication', 'Project Management', 'Deep Learning', 'SQL',
        'Tableau',
...
def ekstrak_edukasi(text):
    ...
    education_keywords = [
        ...
        'Green Technology', 'Ecology', 'Conservation Biology', 'Wildlife Biology', 'Zoology', 'Teknik Informatika', 'Sistem Informasi', 'Ilmu Komputer', 'Manajemen Informatika', 'Teknik Elektro', 
        'Teknik Mesin', 'Teknik Sipil', 'Arsitektur', 'Desain Interior', 'Desain Komunikasi Visual', 
        'Ilmu Komunikasi', 'Hubungan Internasional', 'Hukum', 'Akuntansi', 'Manajemen', 'Ekonomi', 
        'Psikologi', 'Sosiologi', 'Antropologi', 'Pendidikan', 'Sastra', 'Biologi', 'Fisika', 'Kimia', 
        'Matematika', 'Farmasi', 'Kedokteran', 'Keperawatan', 'Ilmu Gizi', 'Agribisnis', 'Agroteknologi', 
        'Teknologi Pangan', 'Peternakan', 'Kehutanan', 'Pariwisata', 'Perhotelan', 'Teknik Lingkungan', 
        'Teknik Industri', 'Teknik Kimia', 'Teknologi Informasi']
...
def pred():
    ...
    if 'resume' in request.files:
        ...
        if filename.endswith('.pdf'):
            ...
            return render_template('index.html', message="Invalid file format. Please upload a PDF or TXT file.")

        predicted_category = prediksi_kategori(text)
        recommended_job = rekomendasi_job(text)
        phone = ekstrak_no_telp(text)
        email = ekstrak_email(text)

        extracted_skills = ektrak_skill(text)
        extracted_education = ekstrak_edukasi(text)
